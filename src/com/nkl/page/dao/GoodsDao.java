...
public class GoodsDao {
	...
	public int addGoods(Goods goods, Connection conn){
		String sql = "INSERT INTO goods(goods_id,goods_no,goods_name,goods_type_id,goods_price,goods_count,alarm_count,goods_desc) values(null,?,?,?,?,?,?,?)";
	...
	public int updateGoods(Goods goods, Connection conn){
		...
		if (goods.getAlarm_count()!=-1) {
			sBuilder.append(" ,alarm_count = " + goods.getAlarm_count() +" ");
		}
	...
	public List<Goods>  listGoodss(Goods goods, Connection conn){
		...
		if (goods.getAlarm_flag()==2) {
			sBuilder.append(" and goods_count <= alarm_count ");
		}
